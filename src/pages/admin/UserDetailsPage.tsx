...
export default function UserDetailsPage() {
  ...
  const { data: userContent, isLoading: contentLoading } = useQuery({
    ...
    queryFn: () => adminService.getUserContent(id!, { type: contentType, page, limit: 10 }),
    enabled: !!id,
  });
    ...
    <div className="space-y-6 p-4 sm:p-6">
          ...
          <div className="mt-6 grid gap-4 border-t border-gray-200 pt-6 dark:border-gray-700 sm:grid-cols-2">
            {user.schoolName && (
              <div>
                ...
                <p className="mt-1 text-gray-900 dark:text-white">{user.schoolName}</p>
              </div>
            )}
            {user.grade && (
              <div>
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Grade</p>
                <p className="mt-1 text-gray-900 dark:text-white">{user.grade}</p>
              </div>
            )}
          </div>
        )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Flashcards</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Attempts</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">User Content</h3>
        </div>
        ...
        {userContent?.meta && userContent.meta.totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-gray-200 px-4 sm:px-6 py-4 dark:border-gray-700">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setPage((p) => Math.min(userContent.meta.totalPages, p + 1))}
      ...
      {user.streak && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="mt-4 flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <span>
              Last activity: {format(new Date(user.streak.lastActivityDate), 'MMMM d, yyyy')}
            </span>
          </div>
        </div>
      )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">
                      {activity.quiz?.title || activity.flashcardSet?.title || 'Unknown'}
                    </p>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-3">
            <button
              onClick={closeModal}
              disabled={deleteUserMutation.isPending || deleteQuizMutation.isPending || deleteFlashcardMutation.isPending || deleteContentMutation.isPending}
            ...
            >
              {(deleteUserMutation.isPending || deleteQuizMutation.isPending || deleteFlashcardMutation.isPending || deleteContentMutation.isPending) ? (
                <>
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></div>
                  Deleting...
                </>
