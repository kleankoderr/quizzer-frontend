...
// Helper to format storage (MB or GB)
...
export default function UserDetailsPage() {
  ...
  const { data: userQuota } = useQuery({
    queryKey: ['userQuota', id],
    queryFn: () => adminService.getUserQuota(id!),
    enabled: !!id,
  });
      ...
      {userQuota && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center justify-between border-b border-gray-200 p-6 dark:border-gray-700">
            ...
            {userQuota.isPremium && (
              <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 px-3 py-1 text-sm font-semibold text-white shadow-sm">
                <svg
                  className="h-4 w-4"
                  fill="currentColor"
                  viewBox="0 0 20 20"
