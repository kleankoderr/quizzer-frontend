...
const ITEM_DELETE_SERVICES = {
    ...
    userDocument: userDocumentService.deleteUserDocument.bind(userDocumentService),
} as const;
...
export const StudyPackDetailsPage: React.FC = () => {
    ...
    const {
        ...
        queryFn: () =>
            id ? studyPackService.getById(id) : Promise.reject(new Error('No ID provided')),
        enabled: !!id,
        retry: false,
    });
    ...
    const handleRemoveItem = useCallback(
        async (itemId: string, type: ItemType) => {
            ...
            // Optimistic update
            queryClient.setQueryData(['studyPack', studyPack.id], (old: StudyPackData | undefined) => {
                if (!old) return old;
                return {
                    ...old,
                    [dataKey]: old[dataKey]?.filter((item: any) => item.id !== itemId) || [],
                };
            });
    ...
    const renderEmptyState = useCallback(
        (tab: TabConfig) => (
            <div
                className="col-span-full py-16 text-center bg-gray-50 dark:bg-gray-800/50 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                <div
                    className="mx-auto w-16 h-16 bg-primary-100 dark:bg-primary-900/30 rounded-full flex items-center justify-center mb-4 text-primary-600 dark:text-primary-400">
    ...
    return (
        <Container>
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div
                                className="flex items-center gap-4 mt-4 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
            ...
            {/* Tabs Navigation */}
            <div role="tablist" className="flex overflow-x-auto border-b border-gray-200 dark:border-gray-700 mb-8 scrollbar-none">
                {tabs.map((tab) => (
                    <button
                        role="tab"
                        key={tab.id}
