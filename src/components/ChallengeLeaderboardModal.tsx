...
export const ChallengeLeaderboardModal: React.FC<
  ...
  useEffect(() => {
    const loadLeaderboard = async () => {
      try {
        setLoading(true);
        const data =
          await challengeService.getChallengeLeaderboard(challengeId);
        setLeaderboard(data);
      } catch (_error) {
      } finally {
        setLoading(false);
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {leaderboard.entries.map((entry) => {
                ...
                const isCurrentUser =
                  leaderboard.currentUser?.userId === entry.userId;
                ...
                return (
                  ...
                  >
                    {/* Rank */}
                    <div
                      className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${rankStyle}`}
                    >
                      {rankIcon}
                    </div>
