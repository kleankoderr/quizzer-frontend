import ReactMarkdown from 'react-markdown';
import React, { useState } from 'react';
import type { QuizQuestion, AnswerValue } from '../types';
import { Check, X, AlertTriangle } from 'lucide-react';

interface QuestionRendererProps {
    question: QuizQuestion;
    questionIndex: number;
    selectedAnswer: AnswerValue | null;
    onAnswerSelect: (answer: AnswerValue) => void;
    showResults?: boolean;
    correctAnswer?: AnswerValue;
    showExplanation?: boolean;
    hideQuestionNumber?: boolean;
}

// Helper function to get button styling based on state
const getButtonClasses = (
    isCorrect: boolean,
    isWrong: boolean,
    isSelected: boolean,
    baseClasses: string = ''
): string => {
    if (isCorrect) {
        return `${baseClasses} border-green-500 bg-green-50 dark:bg-green-900/20 dark:border-green-600`;
    }
    if (isWrong) {
        return `${baseClasses} border-red-500 bg-red-50 dark:bg-red-900/20 dark:border-red-600`;
    }
    if (isSelected) {
        return `${baseClasses} border-blue-500 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 shadow-md scale-[1.02] dark:border-blue-400`;
    }
    return `${baseClasses} border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-500 hover:bg-gray-50 dark:hover:bg-gray-700 hover:shadow-sm bg-white dark:bg-gray-800`;
};

// Helper function to get indicator styling
const getIndicatorClasses = (
    isCorrect: boolean,
    isWrong: boolean,
    isSelected: boolean
): string => {
    if (isCorrect) {
        return 'bg-green-500 text-white';
    }
    if (isWrong) {
        return 'bg-red-500 text-white';
    }
    if (isSelected) {
        return 'bg-blue-500 text-white';
    }
    return 'bg-gray-100 text-gray-700 group-hover:bg-blue-100 group-hover:text-blue-700';
};

export const QuestionRenderer: React.FC<QuestionRendererProps> = ({
                                                                      question,
                                                                      questionIndex,
                                                                      selectedAnswer,
                                                                      onAnswerSelect,
                                                                      showResults = false,
                                                                      correctAnswer,
                                                                      showExplanation = false,
                                                                      hideQuestionNumber = false,
                                                                  }) => {
    const [selectedLeft, setSelectedLeft] = useState<string | null>(null);

    const renderTrueFalse = () => (
        <div className="space-y-3">
            {question.options?.map((option, index) => {
                const isSelected = selectedAnswer === index;
                const isCorrect = showResults && correctAnswer === index;
                const isWrong = showResults && isSelected && selectedAnswer !== correctAnswer;
                const optionId = `tf-${questionIndex}-${option}`;

                return (
                    <button
                        key={optionId}
                        onClick={() => !showResults && onAnswerSelect(index)}
                        disabled={showResults}
                        className={getButtonClasses(
                            isCorrect,
                            isWrong,
                            isSelected,
                            'w-full text-left p-4 sm:p-5 rounded-xl border-2 transition-all duration-200 group'
                        )}
                    >
                        <div className="flex items-start gap-3">
              <span
                  className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors mt-0.5 ${getIndicatorClasses(isCorrect, isWrong, isSelected)}`}
              >
                {option === 'True' ? 'T' : 'F'}
              </span>
                            <span className="text-base text-gray-900 dark:text-white pt-0.5">
                {option}
              </span>
                            {showResults && isCorrect && (
                                <div className="ml-auto flex-shrink-0 flex items-center gap-1 text-green-700 font-semibold">
                                    <Check className="w-5 h-5" />
                                    <span className="hidden sm:inline text-sm">Correct</span>
                                </div>
                            )}
                            {showResults && isWrong && (
                                <div className="ml-auto flex-shrink-0 flex items-center gap-1 text-red-700 font-semibold">
                                    <X className="w-5 h-5" />
                                    <span className="hidden sm:inline text-sm">Your answer</span>
                                </div>
                            )}
                        </div>
                    </button>
                );
            })}
        </div>
    );

    const renderSingleSelect = () => (
        <div className="space-y-3">
            {question.options?.map((option, index) => {
                const isSelected = selectedAnswer === index;
                const isCorrect = showResults && correctAnswer === index;
                const isWrong = showResults && isSelected && selectedAnswer !== correctAnswer;
                const optionId = `ss-${questionIndex}-${index}-${option.substring(0, 10)}`;

                return (
                    <button
                        key={optionId}
                        onClick={() => !showResults && onAnswerSelect(index)}
                        disabled={showResults}
                        className={getButtonClasses(
                            isCorrect,
                            isWrong,
                            isSelected,
                            'w-full text-left p-4 sm:p-5 rounded-xl border-2 transition-all duration-200 group'
                        )}
                    >
                        <div className="flex items-start gap-3">
              <span
                  className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors mt-0.5 ${getIndicatorClasses(isCorrect, isWrong, isSelected)}`}
              >
                {String.fromCodePoint(65 + index)}
              </span>
                            <span className="text-base text-gray-900 dark:text-white pt-0.5">
                {option}
              </span>
                            {showResults && isCorrect && (
                                <div className="ml-auto flex-shrink-0 flex items-center gap-1 text-green-700 font-semibold">
                                    <Check className="w-5 h-5" />
                                    <span className="hidden sm:inline text-sm">Correct</span>
                                </div>
                            )}
                            {showResults && isWrong && (
                                <div className="ml-auto flex-shrink-0 flex items-center gap-1 text-red-700 font-semibold">
                                    <X className="w-5 h-5" />
                                    <span className="hidden sm:inline text-sm">Your answer</span>
                                </div>
                            )}
                        </div>
                    </button>
                );
            })}
        </div>
    );

    const renderMultiSelect = () => {
        const selectedIndices = Array.isArray(selectedAnswer) ? selectedAnswer : [];
        const correctIndices = Array.isArray(correctAnswer) ? correctAnswer : [];

        const toggleOption = (index: number) => {
            if (showResults) return;

            const newSelection = selectedIndices.includes(index)
                ? selectedIndices.filter((i) => i !== index)
                : [...selectedIndices, index];
            onAnswerSelect(newSelection);
        };

        const getMultiSelectClasses = (
            isCorrectlySelected: boolean,
            isWronglySelected: boolean,
            isMissed: boolean,
            isSelected: boolean
        ): string => {
            const baseClass = 'w-full text-left p-4 sm:p-5 rounded-xl border-2 transition-all duration-200 group';

            if (isCorrectlySelected) {
                return `${baseClass} border-green-500 bg-green-50 dark:bg-green-900/20 dark:border-green-600`;
            }
            if (isWronglySelected) {
                return `${baseClass} border-red-500 bg-red-50 dark:bg-red-900/20 dark:border-red-600`;
            }
            if (isMissed) {
                return `${baseClass} border-orange-500 bg-orange-50 dark:bg-orange-900/20 dark:border-orange-600`;
            }
            if (isSelected) {
                return `${baseClass} border-blue-500 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 shadow-md dark:border-blue-400`;
            }
            return `${baseClass} border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-500 hover:bg-gray-50 dark:hover:bg-gray-700 hover:shadow-sm bg-white dark:bg-gray-800`;
        };

        const getCheckboxClasses = (
            isCorrectlySelected: boolean,
            isWronglySelected: boolean,
            isMissed: boolean,
            isSelected: boolean
        ): string => {
            const baseClass = 'flex-shrink-0 w-6 h-6 rounded border-2 flex items-center justify-center text-sm transition-colors mt-0.5';

            if (isCorrectlySelected) {
                return `${baseClass} bg-green-500 border-green-600 text-white`;
            }
            if (isWronglySelected) {
                return `${baseClass} bg-red-500 border-red-600 text-white`;
            }
            if (isMissed) {
                return `${baseClass} bg-orange-500 border-orange-600 text-white`;
            }
            if (isSelected) {
                return `${baseClass} bg-blue-500 border-blue-600 text-white`;
            }
            return `${baseClass} bg-white border-gray-300 group-hover:border-blue-400`;
        };

        return (
            <div className="space-y-3">
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-3 font-medium">
                    Select all that apply:
                </p>
                <div className="space-y-3">
                    {question.options?.map((option, index) => {
                        const isSelected = selectedIndices.includes(index);
                        const shouldBeSelected = correctIndices.includes(index);
                        const isCorrectlySelected = showResults && isSelected && shouldBeSelected;
                        const isWronglySelected = showResults && isSelected && !shouldBeSelected;
                        const isMissed = showResults && !isSelected && shouldBeSelected;
                        const optionId = `ms-${questionIndex}-${index}-${option.substring(0, 10)}`;

                        return (
                            <button
                                key={optionId}
                                onClick={() => toggleOption(index)}
                                disabled={showResults}
                                className={getMultiSelectClasses(
                                    isCorrectlySelected,
                                    isWronglySelected,
                                    isMissed,
                                    isSelected
                                )}
                            >
                                <div className="flex items-start gap-4">
                  <span
                      className={getCheckboxClasses(
                          isCorrectlySelected,
                          isWronglySelected,
                          isMissed,
                          isSelected
                      )}
                  >
                    {(isSelected || isMissed) && '✓'}
                  </span>
                                    <span className="text-base text-gray-900 dark:text-white pt-0.5">
                    {option}
                  </span>
                                    {isMissed && (
                                        <div className="ml-auto flex-shrink-0 flex items-center gap-1 text-orange-700 font-semibold">
                                            <AlertTriangle className="w-5 h-5" />
                                            <span className="hidden sm:inline text-sm">Missed</span>
                                        </div>
                                    )}
                                </div>
                            </button>
                        );
                    })}
                </div>
            </div>
        );
    };

    const renderMatching = () => {
        const correctMatches =
            typeof correctAnswer === 'object' && !Array.isArray(correctAnswer)
                ? correctAnswer
                : {};

        const leftItems =
            question.leftColumn && question.leftColumn.length > 0
                ? question.leftColumn
                : Object.keys(correctMatches);

        const rightItems =
            question.rightColumn && question.rightColumn.length > 0
                ? question.rightColumn
                : Array.from(new Set(Object.values(correctMatches)));

        const hasCorrectAnswer = correctAnswer !== undefined;

        const currentMatches =
            typeof selectedAnswer === 'object' &&
            selectedAnswer !== null &&
            !Array.isArray(selectedAnswer)
                ? selectedAnswer
                : {};

        const handleMatchClick = (leftItem: string, rightItem: string) => {
            if (showResults) return;
            const newMatches = { ...currentMatches, [leftItem]: rightItem };
            onAnswerSelect(newMatches);
        };

        const handleUnselect = (leftItem: string) => {
            if (showResults) return;
            const newMatches = { ...currentMatches };
            delete newMatches[leftItem];
            onAnswerSelect(newMatches);
        };

        const getLeftItemClasses = (
            isCorrect: boolean,
            isWrong: boolean,
            isSelected: boolean,
            hasMatch: boolean
        ): string => {
            const baseClass = 'w-full text-left p-4 rounded-xl border-2 transition-all duration-200';

            if (isCorrect) {
                return `${baseClass} border-green-500 bg-green-50 dark:bg-green-900/20 shadow-md dark:border-green-600`;
            }
            if (isWrong) {
                return `${baseClass} border-red-500 bg-red-50 dark:bg-red-900/20 shadow-md dark:border-red-600`;
            }
            if (isSelected) {
                return `${baseClass} border-blue-500 bg-blue-100 dark:bg-blue-900/30 shadow-lg scale-[1.02] ring-2 ring-blue-300 dark:ring-blue-600 dark:border-blue-400`;
            }
            if (hasMatch) {
                return `${baseClass} border-blue-400 bg-blue-50 dark:bg-blue-900/20 shadow-sm hover:shadow-md cursor-pointer dark:border-blue-500`;
            }
            return `${baseClass} border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-gray-700 hover:shadow-sm cursor-pointer`;
        };

        const getLeftItemBadgeClasses = (
            isCorrect: boolean,
            isWrong: boolean,
            isSelected: boolean,
            hasMatch: boolean
        ): string => {
            const baseClass = 'flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm mt-0.5';

            if (isCorrect) {
                return `${baseClass} bg-green-500 text-white`;
            }
            if (isWrong) {
                return `${baseClass} bg-red-500 text-white`;
            }
            if (isSelected) {
                return `${baseClass} bg-blue-600 text-white`;
            }
            if (hasMatch) {
                return `${baseClass} bg-blue-500 text-white`;
            }
            return `${baseClass} bg-gray-200 text-gray-600`;
        };

        const getRightItemClasses = (
            canSelect: boolean,
            isUsed: boolean
        ): string => {
            const baseClass = 'w-full p-4 rounded-xl border-2 text-left transition-all duration-200';

            if (canSelect) {
                return `${baseClass} border-indigo-400 bg-gradient-to-br from-white to-indigo-50 dark:from-gray-800 dark:to-indigo-900/20 hover:border-indigo-600 dark:hover:border-indigo-500 hover:shadow-lg hover:scale-[1.02] cursor-pointer dark:border-indigo-500`;
            }
            if (isUsed) {
                return `${baseClass} border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 opacity-60 cursor-not-allowed`;
            }
            return `${baseClass} border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 cursor-not-allowed`;
        };

        const getRightItemBadgeClasses = (
            canSelect: boolean,
            isUsed: boolean
        ): string => {
            const baseClass = 'flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm mt-0.5';

            if (canSelect) {
                return `${baseClass} bg-indigo-100 text-indigo-700`;
            }
            if (isUsed) {
                return `${baseClass} bg-gray-300 text-gray-500`;
            }
            return `${baseClass} bg-gray-200 text-gray-400`;
        };

        return (
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                    {/* Left Column */}
                    <div className="space-y-3">
                        <div className="flex items-center gap-2 mb-3">
                            <div className="w-1 h-6 bg-blue-600 rounded-full"></div>
                            <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                Items to Match
                            </h3>
                        </div>
                        {leftItems.map((leftItem, index) => {
                            const userMatch = currentMatches[leftItem];
                            const correctMatch = correctMatches[leftItem];
                            const isCorrect = showResults && hasCorrectAnswer && userMatch === correctMatch;
                            const isWrong = showResults && hasCorrectAnswer && !!userMatch && userMatch !== correctMatch;
                            const isSelected = !showResults && selectedLeft === leftItem;
                            const leftItemId = `match-left-${questionIndex}-${index}-${leftItem.substring(0, 10)}`;

                            return (
                                <button
                                    key={leftItemId}
                                    onClick={() => {
                                        if (showResults) return;
                                        if (userMatch) {
                                            handleUnselect(leftItem);
                                            setSelectedLeft(null);
                                        } else {
                                            setSelectedLeft(leftItem);
                                        }
                                    }}
                                    disabled={showResults}
                                    className={`${getLeftItemClasses(isCorrect, isWrong, isSelected, !!userMatch)} ${showResults ? 'cursor-default' : ''}`}
                                >
                                    <div className="flex items-start gap-3">
                                        <div
                                            className={getLeftItemBadgeClasses(isCorrect, isWrong, isSelected, !!userMatch)}
                                        >
                                            {index + 1}
                                        </div>
                                        <div className="flex-1">
                                            <div className="font-semibold text-gray-900 dark:text-white mb-1">
                                                {leftItem}
                                            </div>
                                            {userMatch && (
                                                <div className="flex items-center gap-2 mt-2 text-sm">
                          <span
                              className={`font-medium px-2 py-1 rounded ${
                                  isCorrect
                                      ? 'bg-green-200 text-green-800'
                                      : isWrong
                                          ? 'bg-red-200 text-red-800'
                                          : 'bg-blue-200 text-blue-800'
                              }`}
                          >
                            → {userMatch}
                          </span>
                                                    {!showResults && (
                                                        <span className="text-xs text-gray-500 italic">
                              (click to change)
                            </span>
                                                    )}
                                                </div>
                                            )}
                                            {isSelected && !userMatch && (
                                                <div className="text-xs text-blue-600 font-medium mt-1 animate-pulse">
                                                    ← Now select a match from the right
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    {showResults && hasCorrectAnswer && isWrong && (
                                        <div className="mt-3 pt-3 border-t border-red-200">
                                            <div className="text-sm text-red-700">
                                                <span className="font-semibold">✗ Your answer:</span>{' '}
                                                {userMatch}
                                            </div>
                                            <div className="text-sm text-green-700 mt-1">
                                                <span className="font-semibold">✓ Correct:</span>{' '}
                                                {correctMatch}
                                            </div>
                                        </div>
                                    )}
                                    {showResults && hasCorrectAnswer && isCorrect && (
                                        <div className="mt-2 text-sm text-green-700 font-semibold">
                                            ✓ Correct match!
                                        </div>
                                    )}
                                </button>
                            );
                        })}
                    </div>

                    {/* Right Column */}
                    <div className="space-y-3">
                        <div className="flex items-center gap-2 mb-3">
                            <div className="w-1 h-6 bg-indigo-600 rounded-full"></div>
                            <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                Possible Matches
                            </h3>
                        </div>
                        {rightItems.map((rightItem, idx) => {
                            const isUsed = Object.values(currentMatches).includes(rightItem);
                            const canSelect = !showResults && selectedLeft && !isUsed;
                            const rightItemId = `match-right-${questionIndex}-${idx}-${rightItem.substring(0, 10)}`;

                            return (
                                <button
                                    key={rightItemId}
                                    onClick={() => {
                                        if (!showResults && selectedLeft && !isUsed) {
                                            handleMatchClick(selectedLeft, rightItem);
                                            setSelectedLeft(null);
                                        }
                                    }}
                                    disabled={showResults || isUsed || !selectedLeft}
                                    className={`${getRightItemClasses(!!canSelect, isUsed)} ${showResults ? 'cursor-default' : ''}`}
                                >
                                    <div className="flex items-start gap-3">
                                        <div
                                            className={getRightItemBadgeClasses(!!canSelect, isUsed)}
                                        >
                                            {String.fromCodePoint(65 + idx)}
                                        </div>
                                        <div className="flex-1">
                                            <div
                                                className={`font-semibold ${isUsed ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-white'}`}
                                            >
                                                {rightItem}
                                            </div>
                                            {isUsed && (
                                                <div className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                                                    <span className="inline-block w-1.5 h-1.5 bg-gray-400 rounded-full"></span>
                                                    Already matched
                                                </div>
                                            )}
                                            {canSelect && (
                                                <div className="text-xs text-indigo-600 font-medium mt-1">
                                                    Click to match →
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                </button>
                            );
                        })}
                    </div>
                </div>
            </div>
        );
    };

    const renderFillBlank = () => {
        const userAnswer = typeof selectedAnswer === 'string' ? selectedAnswer : '';
        const hasCorrectAnswer =
            correctAnswer !== undefined &&
            (typeof correctAnswer === 'string' ||
                (Array.isArray(correctAnswer) && typeof correctAnswer[0] === 'string'));

        let isCorrect = false;
        if (showResults && hasCorrectAnswer) {
            const userNorm = userAnswer.trim().toLowerCase();
            if (Array.isArray(correctAnswer)) {
                const normalizedAnswers = correctAnswer.map((ans) =>
                    String(ans).trim().toLowerCase()
                );
                isCorrect = normalizedAnswers.includes(userNorm);
            } else {
                isCorrect = userNorm === String(correctAnswer).trim().toLowerCase();
            }
        }

        const displayCorrectAnswer = Array.isArray(correctAnswer)
            ? correctAnswer.join(' / ')
            : String(correctAnswer);

        const getInputContainerClasses = (): string => {
            if (!showResults || !hasCorrectAnswer) {
                return 'p-4 rounded-lg bg-white dark:bg-gray-800';
            }
            return isCorrect
                ? 'p-4 rounded-lg bg-green-50 dark:bg-green-900/20'
                : 'p-4 rounded-lg bg-red-50 dark:bg-red-900/20';
        };

        return (
            <div className="space-y-4">
                <div className={getInputContainerClasses()}>
                    <input
                        type="text"
                        value={userAnswer}
                        onChange={(e) => !showResults && onAnswerSelect(e.target.value)}
                        disabled={showResults}
                        placeholder="Type your answer here..."
                        className="w-full p-3 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-0 disabled:bg-gray-100 dark:disabled:bg-gray-700 text-base bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                    />
                </div>
                {showResults && hasCorrectAnswer && (
                    <div className="text-sm space-y-1">
                        {isCorrect ? (
                            <span className="text-green-700 font-semibold">✓ Correct!</span>
                        ) : (
                            <>
                                <div className="text-red-700">
                                    ✗ Your answer: <strong>{userAnswer}</strong>
                                </div>
                                <div className="text-green-700">
                                    ✓ Correct answer: <strong>{displayCorrectAnswer}</strong>
                                </div>
                            </>
                        )}
                    </div>
                )}
            </div>
        );
    };

    return (
        <div>
            <div className="flex items-start gap-2 sm:gap-3 mb-4 sm:mb-6">
                {!hideQuestionNumber && (
                    <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold shadow-md text-sm sm:text-base mt-0.5">
                        {questionIndex + 1}
                    </div>
                )}
                <div className="flex-1 min-w-0">
                    <div className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-900 dark:text-white w-full max-w-full overflow-hidden break-words [&_p]:m-0 [&_pre]:whitespace-pre-wrap [&_code]:break-all [&_code]:bg-gray-100 dark:[&_code]:bg-gray-700 [&_code]:px-1 [&_code]:py-0.5 [&_code]:rounded [&_code]:font-mono [&_code]:text-[0.9em] [&_img]:max-w-full">
                        <ReactMarkdown>{question.question}</ReactMarkdown>
                    </div>
                </div>
            </div>

            {question.questionType === 'true-false' && renderTrueFalse()}
            {question.questionType === 'single-select' && renderSingleSelect()}
            {question.questionType === 'multi-select' && renderMultiSelect()}
            {question.questionType === 'matching' && renderMatching()}
            {question.questionType === 'fill-blank' && renderFillBlank()}

            {showExplanation && question.explanation && (
                <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 sm:p-4">
                    <p className="text-sm font-semibold text-blue-900 dark:text-blue-300 mb-1">
                        💡 Explanation:
                    </p>
                    <div className="text-sm text-blue-800 dark:text-blue-200 [&_strong]:font-bold">
                        <ReactMarkdown>{question.explanation}</ReactMarkdown>
                    </div>
                </div>
            )}
        </div>
    );
};